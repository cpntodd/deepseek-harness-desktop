/**
 * desktop-subscriptions: register OAuth-subscription LLM providers
...
import { OAuthFlowManager, type OAuthAttempt } from './auth/oauth-flow.ts'
import { readFile } from 'node:fs/promises'
import { join } from 'node:path'
import { readClaudeCodeCredentials, refreshClaudeSynced } from './auth/claude-code-creds.ts'
import { registerAuthRpc } from './auth/rpc.ts'
import type {
  AuthController,
  ImageBytesResult,
  ProviderStatus,
...
import { TokenManager, validateModels } from './providers/common.ts'
...
import { createXSearchTool } from './tools/x-search.ts'
...
export type { ModelEntry, ProviderUsage, UsageWindow } from './providers/common.ts'
export type { ProviderStatus } from './auth/rpc.ts'
export type { ClaudeSession, CodexSession, GrokSession, ProviderId } from './auth/store.ts'

export const name = 'desktop-subscriptions'
...
export function apply(ctx: Context, config: Config): void {
  ...
  const onWarn = (message: string): void => {
    ctx.logger.warn(`desktop-subscriptions: ${message}`)
  }
  ...
  if (claudeTokens !== undefined) {
    ...
    ctx.effect(() => () => { clearInterval(syncTimer) }, 'desktop-subscriptions: claude background sync timer')
  }
