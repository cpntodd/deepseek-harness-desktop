...
import {
  ...
  toResponsesInput,
  toResponsesTools,
} from '../src/subscriptions/translate/responses.ts'
import type { ResponsesStreamEvent } from '../src/subscriptions/translate/responses.ts'
import {
  ...
  toAnthropicSystem,
  toAnthropicTools,
} from '../src/subscriptions/translate/anthropic.ts'
import type { AnthropicStreamEvent } from '../src/subscriptions/translate/anthropic.ts'
import { resolveImages } from '../src/subscriptions/translate/resolved.ts'
...
/** Assert a synchronous call throws an LlmError carrying the given code. */
function expectLlmThrow(fn: () => unknown, code: string): void {
  let caught: unknown
  try {
    fn()
  } catch (error) {
  ...
  if (caught instanceof LlmError) expect(caught.code).toBe(code)
}

describe('subscriptions translate', () => {
  it('toResponsesInput: text, tool call, and tool result round trip', () => {
  ...
  it('toResponsesInput: system-role messages become instructions unless options.system wins', () => {
    const systemMessage = message('system', [{ type: 'text', text: 'from history' }])
    const fromMessages = toResponsesInput([systemMessage])
    expect(fromMessages.instructions).toBe('from history')
    expect(fromMessages.input).toEqual([])
    const explicit = toResponsesInput([systemMessage], 'explicit system')
    expect(explicit.instructions).toBe('explicit system')
  })

  it('toResponsesTools maps to Responses function tools', () => {
    expect(toResponsesTools([{ name: 'bash', description: 'run', parameters: { type: 'object' } }])).toEqual([
      { type: 'function', name: 'bash', description: 'run', parameters: { type: 'object' } },
    ])
  })

  it('toResponsesInput: resolved image parts become input_image data URLs', () => {
  ...
  it('resolveImages: passthrough, loud failure without attachments, and resolution', async () => {
    const plain = [message('user', [{ type: 'text', text: 'hi' }])]
    expect(await resolveImages(plain, undefined)).toBe(plain)

    const withImage = [message('user', [{
      type: 'image',
      attachment: { attachmentId: 'a1', mediaType: 'image/png', bytes: 3, width: 1, height: 1 },
    } as never])]
    await expect(resolveImages(withImage, undefined)).rejects.toSatisfy(
    ...
    expect(resolved[0]?.content).toEqual([{ type: 'image', mediaType: 'image/png', dataBase64: 'aGk=' }])
  })

  it('Responses translator: text + tool call stream yields usage before finish', () => {
  ...
  it('Responses translator: plain text completion finishes with stop', () => {
    ...
    expect(chunks.at(-2)).toEqual({ type: 'usage', usage: { inputTokens: 10, outputTokens: 2 } })
    expect(chunks.at(-1)).toEqual({ type: 'finish', reason: { kind: 'stop' } })
  })

  it('Responses translator: empty completion is an EMPTY_RESPONSE error finish', () => {
    const chunks = drain(new ResponsesStreamTranslator(), [
    ...
    expect(usageIndex >= 0 && usageIndex < chunks.length - 1).toBeTruthy()
  })

  it('Responses translator: response.failed maps context overflow and quota', () => {
  ...
  it('toAnthropicMessages: merge, tool_use input parsing, tool_result', () => {
    ...
    expect(malformed[0]?.content[0]).toEqual({ type: 'tool_use', id: 'c', name: 'n', input: {} })
  })

  it('toAnthropicMessages: resolved image parts become base64 image blocks', () => {
    const messages = toAnthropicMessages([{
      role: 'user',
      content: [
        { type: 'image', mediaType: 'image/png', dataBase64: 'aGk=' },
  ...
  it('toAnthropicSystem: Claude Code identity first, then explicit and history system text', () => {
    const blocks = toAnthropicSystem('explicit', [message('system', [{ type: 'text', text: 'from history' }])])
    expect(blocks).toEqual([
      { type: 'text', text: CLAUDE_CODE_IDENTITY },
      { type: 'text', text: 'explicit' },
      { type: 'text', text: 'from history' },
    ])
    expect(toAnthropicSystem().length).toBe(1)
  })

  it('toAnthropicTools maps to input_schema tools', () => {
    expect(toAnthropicTools([{ name: 'bash', description: 'run', parameters: { type: 'object' } }])).toEqual([
      { name: 'bash', description: 'run', input_schema: { type: 'object' } },
    ])
  })

  it('Anthropic translator: text + tool_use stream with usage before finish', () => {
  ...
  it('Anthropic translator: stop reasons and empty completion', () => {
    ...
    expect(maxed.at(-1)).toEqual({ type: 'finish', reason: { kind: 'max-tokens' } })
    ...
    expect(usageIndex >= 0 && usageIndex < empty.length - 1).toBeTruthy()
  })

  it('Anthropic translator: error event mapping', () => {
