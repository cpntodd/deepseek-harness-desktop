...
import { catalogStore, sanitizeSnapshot } from '../src/subscriptions/providers/catalog-store.ts'

async function tempStorePath(): Promise<string> {
  return join(await mkdtemp(join(tmpdir(), 'dsh-models-')), 'models.json')
}

describe('subscriptions catalog store', () => {

it('catalog store round-trips per-provider snapshots in one file', async () => {
  ...
  expect(loaded?.at).toBe(123)
  expect(loaded?.models[0]?.reasoning?.defaultEffort).toBe('high')
  expect(loaded?.models[0]?.reasoning?.efforts[0]?.description).toBe('extensive')
  expect((await codex.load())?.models[0]?.name).toBe('GPT-5.2 Codex')

  // Clearing one provider keeps the other.
  await grok.clear()
  expect(await grok.load()).toBeUndefined()
  expect((await codex.load())?.at).toBe(456)
  // The file itself stays valid JSON (parse throws otherwise).
  JSON.parse(await readFile(path, 'utf8'))
})

it('catalog store tolerates missing and corrupt files', async () => {
  const path = await tempStorePath()
  ...
  expect((await store.load())?.at).toBe(2)
})

it('sanitizeSnapshot drops malformed snapshots wholesale', () => {
  const model = { id: 'g', name: 'G' }
  const efforts = [{ id: 'high', name: 'High' }]
  // A valid snapshot passes through.
  expect(sanitizeSnapshot({ at: 1, models: [model] })).toBeDefined()
  expect(sanitizeSnapshot({ at: 1, models: [{ ...model, reasoning: { efforts, defaultEffort: 'high' } }] })).toBeDefined()
  ...
  expect(sanitizeSnapshot({ at: 1, models: [{ ...model, fastTier: 'yes' }] })).toBeUndefined()
  for (const malformed of [
    ...
    expect(sanitizeSnapshot(malformed)).toBeUndefined()
  }
})
})
